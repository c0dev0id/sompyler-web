# -*- coding: utf-8 -*-

from .bezier_gradient import plot_bezier_gradient
from .point import Point, BezierEdgePoint, SympartialPoint
from ...syntaxtracer import default_noop, deeper_level
from Sompyler.limits import observe_computing_resources
import re
import copy
from operator import itemgetter

syntracer = default_noop()

_sentinel = object()
class Shape:
    __slots__ = ['length', 'coords', 'y_max', 'base']
    def __init__(self, span, *coords):

        self.length = span[0]

        # scale x to tune length, y to 1
        x_max = coords[-1][0]
        y_max = span[1] or max( i[1] for i in coords )

        try:
            base, y_max = y_max
        except TypeError:
            base = 0

        if not y_max:
            raise ShapeDefinitionError("y_max cannot not be 0 [" + repr(coords) + "]")

        self.y_max = abs(y_max)
        self.base = base

        y_div = max(abs(base + y_max), abs(base))
        def y(c):
            return (base + y_max * c / abs(y_max)) / y_div

        if coords[0][0] > 0:
            try:
                _, y_head = span[1]
            except TypeError:
                y_head = span[1]
            self.coords = [ BezierEdgePoint(0, y(abs(y_head)), span[2], False) ]
        else:
            self.coords = []

        x_last = -1

        for n, i in enumerate(coords):
            if not x_last < i[0]:
                raise ShapeDefinitionError(
                    "Wrong order in coordinates: {} not less than {} in {}".format(
                        x_last, i[0], coords
                    )
                )

            x_last = i[0]

            x = i[0] / x_max

            # if i[1] < 0:
            #     raise Exception("y must not be less than 0")

            ext = i[3]

            if ext is None:
                ext = False
            elif isinstance(ext, float):
                pointClass = BezierEdgePoint
                if self.coords:
                    prior = coords[n-1] if n else self.coords[0]
                    if n < len(coords)-1:
                        nbp = coords[n+1]
                        d = orthogonal_distance_C_to_ABsecant( prior, nbp, i )
                        d0 = Point.distance( prior, nbp )
                        ext = d / (d + d0) > ext
                    else:
                        ext = True
                else:
                    ext = True
            elif isinstance(ext, bool):
                pointClass = BezierEdgePoint
            elif isinstance(ext, object):
                pointClass = SympartialPoint
            else:
                raise ShapeAssertError(
                    "Type of extension not supported: " + ext
                )
            if ext is None:
                raise ShapeAssertError( "ext is None" )

            self.coords.append( pointClass( x, y(i[1]), i[2], ext ) )

    def iterate_coords(self, offset=_sentinel, *, tuple_with_neighbourhood=False):

        if offset is _sentinel:
            # coroutine mode
            rx = yield [ i.x for i in self.coords ]
            twn = tuple_with_neighbourhood
            offset = 0
        else:
            if tuple_with_neighbourhood:
                raise RuntimeError(
                    'iterate_coords: tuple_with_neighbourhood=True only supported '
                    'if offset argument is *not* given.'
                )
            twn = tuple_with_neighbourhood
            rx = None

        it = iter(self.coords[offset:])
        last = n = None
        length = self.length
        nx = 0

        def forward():
            nonlocal last, n, nx
            last = n
            n = next(it)
            nx = n.x * length
            n = n.new_alike(nx)

        forward()

        while True:
            if rx is None:
                rx = nx
            if rx == nx:
                rx = yield (n, n, n) if twn else n
                try: forward()
                except StopIteration:
                    return
            elif rx < nx:
                if last is None:
                    i = n.new_alike(rx)
                    rx = yield (last, i, n) if twn else i
                else:
                    d = (rx - last.x) / (nx - last.x)
                    i = last.weighted_average(last, d, n)
                    rx = yield (last, i, n) if twn else i
            else:
                try:
                    forward()
                    continue
                except StopIteration:
                    i = last.new_alike(rx)
                    rx = yield (last, i, None) if twn else last


    def y_slice (self, offset, until, step):
        s = slice(offset, until, step)
        return tuple( c.y for c in self.coords[s] )

    def rescale_y (self, other_y_max):
        adj_y = self.y_max / other_y_max
        for i in self.coords:
            i.y *= adj_y
        self.y_max = other_y_max

    @classmethod
    def from_string(cls, string):
        """
        Shape.from_string("length:[y0;]x1,y1[!];x2,y2[!];...")

        Define the attack, sustain or release shape of the envelope as a bezier
        curve, that is, the coordinates of its outer polygon.

        Attributes:
          * length - the duration of the part of the envelope, in seconds
          * y0 - the y value of point at x=0. Default is 0.

        If an exclamation mark is appended, the bezier edge will be sharp.

        """

        bezier = string

        if ':' in bezier:
            coords, bezier = bezier.split(":",1)
            if coords.isdecimal():
                coords = int(coords)
            else:
                coords = float(coords)
            coords = [[ coords, 0, 1 ]] # list with 1 item
        else:
            coords = [[ 1, 0, 1 ]]

        m = re.match(r"(-?\d+)??([+-]?\d+)(?:\*(\d+))?;", bezier)
        coords[0][1] = int(m.group(2)) if m else 0

        if m:
            _, bezier = bezier.split(";",1)
            if m.group(1):
                coords[0][1] = (
                        int(m.group(1) or 0), coords[0][1]
                    )
            coords[0][2] = int(m.group(3) or 1)

        syntracer("shape",
            length=coords[0][0],
            start=m.group(0)[:-1] if m else 0,
            z=coords[0][2]
        )

        with deeper_level("shape"):
            try:
                for m0 in re.split(r";\s*", bezier):
                    x, y = tuple( m0.split(",", 1) )
                    if re.search(r'!$', y):
                        y = y[:-1]
                        s = True
                    else:
                        s = False
                    m = re.search(r'(?<=\d)\*(\d+)', y)
                    if m:
                        z = m.group(1)
                        y = y[:-len(z)-1]
                    else:
                        z = 1
                    syntracer("coords", x=x, y=y, z=z, is_sharp=s)
                    coords.append([ float(x), float(y), int(z), s ])
            except (ValueError,TypeError) as e:
                raise ShapeDefinitionError(f"{m0}: illegal syntax") from e

        return cls(*coords)

    def render (
        self, unit_length=1, is_length_factor=True, y_scale=1,
        adj_length=False, tail_shape=None, add_plateau=None
    ):
        """
        Get samples along the bezier curve determined by the edge coordinates
          in self.coords.

        Parameters:
          unit_length       How many samples to render per self.length unit
          is_length_factor  If false, result array will be unit_length long
          y_scale           If true, points will be vertically scaled to
                            self.y_max
          adj_length        Edge coordinates will be extended or truncated to
                            match this parameter
          tail_shape        In case you want the result terminate at a minimum
                            or maximum level, depending on whether it starts
                            from the top or from the bottom. The tail shape is
                            used from the first intersection with self. So, if
                            tail and self shapes do not intersect, this
                            parameter is ignored.
        """

        base_length = self.length if is_length_factor else 1

        if y_scale is True:
            y_scale = self.y_max

        if adj_length is True:
            base_length *= y_scale
            coords = self.new_coords(y_scale=y_scale) 
        else:
            coords = self.new_coords(adj_length, y_scale)
            if adj_length: base_length = adj_length

        if add_plateau and add_plateau[1] < 1:
            coords = Shape.coords_with_plateau(coords,
                add_plateau[0] / base_length, add_plateau[1]
            )
            base_length += add_plateau[0]

        length = int( round(unit_length * base_length) )
        if length:
            observe_computing_resources(length)
        else:
            return []

        if tail_shape:
            extreme  = min if tail_shape.coords[0].y else max
            tbcoords = [coords[-1].y]
            tbcoords.extend(c.y for c in tail_shape.coords)
            if extreme(tbcoords) != tbcoords[0]:
                coords = Shape.coords_with_tail(
                    tail_shape, coords, base_length
                )

        # A sharp edge of a bezier curve always corresponds to a point of the
        # graph. So we need to make segments the start and end points of which
        # are sharp edges, in the way that the end point of a segment is
        # identical to the start point of the successor.
        last = coords[0]
        lasti = 1
        segments = []
        while lasti < len(coords):
            segment = [ last ]
            for c in coords[lasti:]:
                segment.append(c)
                lasti += 1
                if c.is_sharp:
                    break
            segments.append( segment )
            last = c

        # We will now divide and distribute the requested total length in
        # samples. Let us round in two phases. First we save the floor values,
        # then we increment selected ones by one. Selected are those which
        # are closest to their successor ordinal. Where that rule leads to
        # ambiguity, the last of each group of values with equal rational part
        # is selected.
        tmp_length_calc = list(reversed( sorted(
            (
                (s, ( s[-1].x - s[0].x ) / coords[-1].x * length)
                    for s in segments
            ),
            key=lambda t: t[1] - int( t[1] )
        )))
        lengths = dict( ( id( t[0] ), int( t[1] ) ) for t in tmp_length_calc )
        remainder = length - sum( lengths.values() )
        for i in tmp_length_calc[:remainder]:
            lengths[ id( i[0] ) ] += 1

        results = []
        for s in segments:
            length = lengths[ id(s) ]
            if length == 0:
                continue
            results.extend( plot_bezier_gradient( length, *s ) )

        return results

    def new_coords(self, adj_length=None, y_scale=1, reverse=False):
        """
        Return new, independent coordinate points that span original length
        or a specified one.

        If specified length is less than the original, coordinates are ignored
        from the end. The last is adjusted linearly.

        If specified length is more than original, coordinates are added at
        the tail, linearly advancing in the direction defined last. When
        it hits the y-axis, it proceeds in horizontal direction, because
        negative values are not allowed.
        """

        if adj_length:
            if adj_length < 0:
                raise ShapeAssertError("Negative shape length")
            adj_length /= self.length 
        elif y_scale == 1:
            coords = self.coords
            if reverse: return reversed(coords)
            else: return coords

        coords = self.coords[:]

        if adj_length:

            if adj_length > 1:
                pult_x = coords[-2].x
                pult_y, ult_y = (i.y for i in coords[-2:])
                ult_rise = (ult_y - pult_y) / (1 - pult_x)
                fill_x = adj_length
                fill_y = ult_y + (fill_x - 1) * ult_rise
                fill_x2 = None
                if fill_y < 0:
                    # even out fall into negative to being constant 0
                    fill_x2 = fill_x
                    fill_x = -ult_y / ult_rise + 1
                    fill_y = 0
                replaced = coords.pop(-1)
                coords.append(replaced.new_alike( fill_x, fill_y ))
                if fill_x2: coords.append(
                    replaced.new_alike( fill_x2, fill_y )
                )

            elif adj_length < 1:
                n = 0
                for i in coords:
                    if adj_length >= i.x:
                       n += 1
                    else: break
                h = coords[n-1]
                new_coord = h.weighted_average(
                    h, (adj_length - h.x) / (i.x - h.x), i
                )
                del coords[n:]
                coords.append(new_coord)

        # rescale x-dimension to new maximum, y-dimension to original
        # y_max * y_scale
        x_max = coords[-1].x
        return [ c.new_alike(c.x / x_max, c.y * y_scale) for c in coords ]


    @staticmethod
    def coords_with_tail( tail_shape, coords, length ):

        tail_length = tail_shape.length
        tail_x_offset = length - tail_length

        def adj_length(c):
            return c.new_alike((tail_x_offset + c.x * tail_length) / length)

        def line_segment_progressor(coords, x_offset=None):
            old = coords[0]
            for i in coords[1:]:
                yield (old, i)
                old = i

        if tail_x_offset < 0: # we cut tail short from the front
            tail_coords = list(reversed(
                    tail_shape.new_coords(length, reverse=True)
                ))
            tail_x_offset = 0
        elif tail_x_offset > 0:
            tail_coords = list(
                adj_length(c) for c in tail_shape.coords
            )
        else:
            tail_coords = tail_shape.coords

        progress_self = line_segment_progressor(coords)
        progress_tail = line_segment_progressor(tail_coords)

        is_flags = 0
        take_sustain = []
        while not is_flags == 3:

            if not is_flags & 2:
                try:
                    s_begin, s_end = next(progress_self)
                    take_sustain.append(s_begin)
                except StopIteration:
                    raise RuntimeError(
                        "Cannot handle case where sustain and tail "
                        "do not intersect"
                    )
            if not is_flags & 1:
                try:
                    t_begin, t_end = next(progress_tail)
                except StopIteration:
                    return coords

            is_flags = 0
            if s_end.x <= t_end.x < 1:
                is_flags |= 1
            else:
                is_flags |= 2
                s_a = s_begin.y - s_end.y
                s_b = s_end.x - s_begin.x
                s_c = s_end.x * s_begin.y - s_begin.x * s_end.y

                t_a = t_begin.y - t_end.y
                t_b = t_end.x - t_begin.x
                t_c = t_end.x * t_begin.y - t_begin.x * t_end.y

                orient = s_a * t_b - t_a * s_b
                if orient == 0:
                    if s_a * t_begin.x + s_b * t_begin.y != s_c:
                        # Lines do not match
                        raise RuntimeError("Lines are parallel")
                else:
                    xs = (s_c * t_b - t_c * s_b) / orient
                    if xs <= t_end.x:
                        is_flags |= 1
                        take_sustain.append(
                            t_begin.weighted_average(
                                t_begin,
                                max((xs - t_begin.x) / (t_end.x - t_begin.x), 0),
                                t_end
                            )
                        )

        take_sustain.append(t_end)
        for _, p in progress_tail:
            take_sustain.append(adj_length(p))

        return take_sustain

    @classmethod
    def weighted_average (cls, left, dist, right, *,
            orig_form_length_dependent=True):
        """
        Assuming a smooth transition to a right curve in a distance,
        Shape.weighted_average(left_shape, distance, right_shape) returns an
        intermediate shape at a given position < 1 and > 0.
        """

        if dist < 0 or dist > 1:
            raise ShapeAssertError("dist = " + str(dist))

        def avg(a, b): return (1 - dist) * a + dist * b

        adj_length = avg( left.length, right.length )
        if isinstance(left.length, int) and isinstance(right.length, int):
            # This method does not round well when multiple weighted_average
            # calls are applied in a stack (multi-shape definitions in a
            # protopartial).
            adj_length = int(round(adj_length))

        if orig_form_length_dependent:
            lcoords = left.new_coords(adj_length)
            rcoords = right.new_coords(adj_length)
        else:
            coords = []
            for shape in [left, right]:
                coords.append(shape.new_coords())
                for c in coords[-1]:
                    c.x *= shape.length / adj_length
            lcoords, rcoords = coords

        try:
            ymax_div = avg(
                max(c.y for c in left.coords) / max(c.y for c in lcoords),
                max(c.y for c in right.coords) / max(c.y for c in rcoords),
            )
        except ZeroDivisionError:
            # May occur when left or right is evenly at null
            # Ex: pseudo frequency variation shape -1+2:0,1;1,1 to be 
            # average with real frequency variations.
            ymax_div = 1

        if not len(lcoords) == len(rcoords):
            avg_coords_num = int( avg( len(lcoords), len(rcoords) ) + .5 )
            lcoords = cls._adjust_coords_num(lcoords, avg_coords_num)
            rcoords = cls._adjust_coords_num(rcoords, avg_coords_num)

        assert len(lcoords) == len(rcoords)

        y_max = avg(left.y_max, right.y_max)

        coords = []
        for i in range( len(lcoords) ):
            l = lcoords[i]
            r = rcoords[i]
            c = l.weighted_average( l, dist, r )
            c.y *= y_max * ymax_div
            coords.append(c)

        return cls( (adj_length, y_max / ymax_div, 1), *coords )

    def edgy (self):
        return (self.coords[0].y, self.coords[-1].y)

    def adjust_coords_num (self, num):
        """
        Adjust number of coordinates so two shapes have the same number and can
        therefore be meaned.

        When the number is raised, new nodes are inserted in equal distances
        where the distance between other nodes is greatest.
        When it is reduced, nodes are dropped of which the distance to the
        thought line connecting the closest neighbouring nodes is least.
        """
        self.coords = Shape._adjust_coords_num( self.coords, num )

    @classmethod
    def _adjust_coords_num (cls, coords, num):
        # Due to algorithmic weaknesses, We occasionally need more than one
        # iteration approaching the requested number of nodes.
        while num > len(coords):
            coords = cls._raise_coords( coords, num - len(coords) )
        while num < len(coords):
            coords = cls._lessen_coords( coords, len(coords) - num )

        if len(coords) != num:
            raise ShapeAssertError(
                "number of nodes expected to be {} but is actually {}".format(
                        num, len(coords)
                    )
                )

        return coords

    def lessen_coords (self, by_num):
        self.coords = self._lessen_coords( self.coords, by_num)

    @classmethod
    def _lessen_coords (cls, coords, by_num):

        distances = {}
        for i, a in enumerate(coords[:-2]):
            c = coords[i+1]
            b = coords[i+2]
            distances[c] = orthogonal_distance_C_to_ABsecant(a, b, c)

        kept_dist = sorted(distances.items(), key=itemgetter(1))[by_num:]
        kept_dist.append( (coords[0], 1) )
        kept_dist.append( (coords[-1], 1) )
        distances = dict( kept_dist )

        return [ c for c in coords if c in distances ]

    def raise_coords (self, by_num):
        self.coords = self._raise_coords( self.coords, by_num)

    @classmethod
    def _raise_coords (cls, coords, by_num):

        distances = {}
        c_last = coords[0]

        new_coords = [c_last]

        for c in coords[1:]:
            distances[c] = Point.distance(c_last, c)
            c_last = c

        sum_dist = sum( d for d in distances.values() )

        remainders = []
        rem_by_num = by_num
        for (c, dist) in distances.items():
             scaled = dist / sum_dist * by_num
             int_scaled = int(scaled)
             remainders.append( (c, scaled - int_scaled) )
             distances[c] = int_scaled
             rem_by_num -= int_scaled

        for (c, _) in sorted(
            remainders, key=itemgetter(1), reverse=True
        )[0:rem_by_num]:
            distances[c] += 1

        sum_coords = 0
        for i, c in enumerate(coords[1:]):
            inter_coords = distances[c]
            sum_coords += inter_coords

            xlen = c.x - coords[i].x

            for step in range(inter_coords):
                new_coords.append( coords[i].weighted_average(
                    coords[i], (step+1) / (inter_coords+1), c
                ) )

            new_coords.append(c)

        assert sum_coords == by_num

        return new_coords

    def derive ( self, other ):

        coords = []
        old_coords = iter(self.coords)
        new_coords = iter(other.coords)
        i = next(new_coords)
        j = next(old_coords)

        while True:
            if j.x > i.x:
                coords.append(i)
                i = next(new_coords, None)
                if i is None: break
            else:
                coords.append(j)
                j = next(old_coords, None)
                if j is None: break

        coords.extend( [x for x in old_coords] )
        coords.extend( [x for x in new_coords] )

        return Shape( self.lengh, *coords )

    def __repr__(self):
        return self._base_repr() + ")"

    def _base_repr(self, orig_y=False):

        def mindiff(g):
            last = next(g)
            diffs = []
            try:
                while True:
                    c = next(g)
                    d = abs(c - last)
                    if d: diffs.append(d)
                    last = c
            except StopIteration:
                return min(diffs) if diffs else 1

        mindiff_x = mindiff(c.x for c in self.coords)
        mindiff_y = 1 if orig_y else mindiff(c.y for c in self.coords)
        y_max = self.y_max if orig_y else max(c.y for c in self.coords)
        y_prec = str(5 if orig_y else 0)

        return ("{}(({}, {:." + y_prec + "f}), {}").format(
                self.__class__.__name__,
                self.length,
                y_max / mindiff_y,
                ", ".join(("({:.0f}, {:." + y_prec + "f}, {!r})").format(
                    c.x/mindiff_x, c.y/mindiff_y, c.ext()
                ) for c in self.coords)
            )

    @staticmethod
    def coords_with_plateau(coords, duration, position):
        """ insert a plateau of a pair of nodes with same y at
            relative position {position}.
            duration must be relative float to original length.
        """

        last_c = None
        for i, c in enumerate(coords):
            if c.x <= position:
                last_c = c
            else: break

        front, tail = coords[:i], coords[i:]
        if c.x > position:
            c0 = BezierEdgePoint.weighted_average(
                    last_c, (position - last_c.x) / (c.x - last_c.x), c
                )
            front.append(c0)
        front.append(c.new_alike(x=last_c.x+duration))
        for c in tail:
            c.x += duration
            last_c = c

        coords = front + tail
        for c in coords: c.x /= last_c.x

        return coords

def orthogonal_distance_C_to_ABsecant(a, b, c):
    n_ab = (b.y - a.y) / (b.x - a.x)
    if n_ab:
        n_dc = (a.x - b.x) / (b.y - a.y)
        x = (c.y - n_dc*c.x - b.y + n_ab*b.x) / (n_ab - n_dc)
    else: # work around division by zero
        x = c.x
    return Point.distance( Point(x, n_ab*(x - b.x) + b.y), c )


class ShapeDefinitionError(RuntimeError):
    pass

class ShapeAssertError(AssertionError):
    pass
