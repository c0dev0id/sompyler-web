...
- **Python core now runs the full render pipeline.** When `core: python` is active, `startRender()` hands the entire score to Python's `play()` function via Pyodide — including mixing, room reverb, and amplitude normalisation — producing output identical to `sompyle score.spls out.wav` on the CLI. JS only prepares input files (stripping web-only extensions with warnings logged per key) and feeds the result to the player. RFC tap-rooms are forwarded to Python; freeverb rooms are not supported and render dry with a warning. Solo render stays on the per-note path.
...
- **Instrument preview phaser effect on FM kick.** The preview used a fixed 2-second damp tail regardless of instrument type. Short-envelope FM instruments (kick, bass drum) produced a 2-second FM sweep that sounded like a phaser firing. Damp is now capped to `min(2, release + 0.15)` so short releases don't produce long synthesizer sweeps.
